...
WALK = {
    "en": {
        ...
        "steps": [
            ...
            ], "# Debian, Ubuntu\nsudo apt install postgresql postgresql-server-dev-all libpq-dev\n\n# macOS with Homebrew\nbrew install postgresql@17\nbrew services start postgresql@17\n\npsql --version\npg_config --includedir\npg_isready", "psql (PostgreSQL) 17.x\n/usr/include/postgresql\n/var/run/postgresql:5432 - accepting connections", [
                "PostgreSQL is a <em>server</em>: a program that runs in the background and holds any number of databases, each with its own tables. <code>psql</code> is the terminal that talks to it. On Debian and Ubuntu the package starts the server for you; <code>pg_isready</code> says whether it is up.",
                "<strong>If it does not:</strong> <em>no response</em> or <em>connection refused</em> means the server is not running — <code>sudo systemctl start postgresql</code> on Linux, <code>brew services start postgresql@17</code> on macOS.",
            ...
            ("Create the database", [
                "Your first database, in two commands. <code>createdb blog</code> asks the server for a new, empty database named <code>blog</code>. <code>psql -d blog -f blog.sql</code> opens it and runs the compiled file in it: the file's <code>CREATE</code> statements make the enum type and the two tables, and <code>psql</code> answers one line per statement. Then <code>\\dt</code> lists the tables — <code>-c</code> runs one command and comes back to the shell.",
            ], "createdb blog\npsql -d blog -f blog.sql\npsql -d blog -c '\\dt'", "CREATE TYPE\nCREATE TABLE\nCREATE TABLE\nPREPARE\nPREPARE\nPREPARE\nPREPARE\nPREPARE\nPREPARE\n         List of tables\n Schema | Name | Type  |  Owner\n--------+------+-------+--------\n public | post | table | you\n public | user | table | you\n(2 rows)", [
                "<strong>If it does not:</strong> <em>database \"blog\" already exists</em> — you ran it twice; <code>dropdb blog</code> throws it away, rows included, and you start over. <em>permission denied to create database</em> or <em>role … does not exist</em> — step 4. <em>connection to server … failed</em> — step 3, the server is not running.",
                "The <code>CREATE</code> statements are permanent. The <code>PREPARE</code> statements are not: they live for the <code>psql</code> session that ran them. That is why the next step loads them again, with <code>--queries</code>, in the session that uses them — and why a program (step 11) prepares its own.",
            ]),
            ("Put some rows in, and query them", [
                "Now a <code>psql</code> session you stay in. The first line, in the shell, writes the queries alone to a file — <code>--queries</code> prints the <code>PREPARE</code>s without the <code>CREATE</code>s, which would fail on tables that exist. <code>psql -d blog</code> then opens the database and gives you its own prompt, <code>blog=&gt;</code>: what follows that prompt is typed inside <code>psql</code>, and each command ends with a semicolon. <code>\\i</code> reads the file of queries into the session; the two <code>INSERT</code>s put two users and three posts in; <code>EXECUTE</code> calls a prepared statement by name — the file's names, in <code>snake_case</code>; the last <code>SELECT</code> shows the table as it is; <code>\\q</code> leaves.",
            ], "kealsql --queries blog.kealsql > queries.sql\npsql -d blog\n\nblog=> \\i queries.sql\nblog=> INSERT INTO \"user\" (name, email) VALUES ('ada', 'ada@x'), ('bob', 'bob@x');\nblog=> INSERT INTO post (author, editor, title, status, created) VALUES\n         (1, 2, 'Hello', 'Published', '2026-01-01'),\n         (1, NULL, 'Draft one', 'Draft', '2026-01-02'),\n         (2, NULL, 'Bob post', 'Published', '2026-01-03');\nblog=> EXECUTE by_author('ada');\nblog=> EXECUTE editor_of(2);\nblog=> EXECUTE drafts;\nblog=> SELECT id, author, title, status FROM post;\nblog=> \\q", " id | title\n----+-------\n  1 | Hello\n(1 row)\n\n name\n------\n\n(1 row)\n\n count\n-------\n     1\n(1 row)\n\n id | author |   title   |  status\n----+--------+-----------+-----------\n  1 |      1 | Hello     | Published\n  2 |      1 | Draft one | Draft\n  3 |      2 | Bob post  | Published\n(3 rows)", [
                "<strong>If it does not:</strong> <em>prepared statement \"by_author\" does not exist</em> — this is a new session, the <code>PREPARE</code>s of the last one are gone: <code>\\i queries.sql</code> again. <em>duplicate key value violates unique constraint</em> — the rows are already in; go on. A prompt that turns into <code>blog-&gt;</code> is waiting for the semicolon you forgot.",
            ...
            ], ("app.keal", "import \"./blog.kealsql\"\n\n// The database and its tables, made if they are missing; a connection to them.\nval db = createBlog(\"\", \"blog\")\n\nprintln(\"drafts: ${db.drafts()}\")\nfor (p in db.byAuthor(\"ada\")) {\n    println(\"#${p.id} ${p.title}\")\n}\nval editor = db.editorOf(1)\nprintln(\"editor of #1: ${editor ?: \"none\"}\")\n\n// A change: the draft goes out, and its row comes back as a record.\nval post = db.publish(2)\nprintln(\"published: ${post.headline()}\")\nprintln(\"drafts now: ${db.drafts()}\")\nfor (p in db.byAuthor(\"ada\")) {\n    println(\"#${p.id} ${p.title}\")\n}\ndb.close()"), None, [
                "<code>createBlog(\"\", \"blog\")</code>: the first argument is a libpq connection string — empty, it uses the same defaults as <code>psql</code> (your user, the local server); the second is the database's name, made if it does not exist. Here it exists, with rows, so the call only connects. Every <code>func</code> and <code>proc</code> of the file is a method on <code>db</code>, with the parameters and the result the file declares.",
                "<code>db.drafts()</code> answers an <code>Int</code>. <code>db.byAuthor(\"ada\")</code> answers a list of records with <code>id</code> and <code>title</code>. <code>db.editorOf(1)</code> answers a <code>String?</code>, which is why <code>?: \"none\"</code> is there. <code>db.publish(2)</code> is the change: it runs the <code>UPDATE</code> and answers the row it changed, as a <code>Post</code> — a record with the table's columns and the <code>headline()</code> method the table declares. Nothing here is SQL: a column the file does not have, or a wrong type, stops the build.",
            ]),
            ("Build and run it", [
                ...
                "Ada's second post was a draft, so <code>byAuthor</code> did not list it; published, it comes first, being the newer. Run <code>./app</code> again: the first line now says <code>drafts: 0</code>, because the change was written to the database — it is not the program's memory. <code>psql -d blog -c 'SELECT * FROM post'</code> shows it from the other side, any time.",
                ...
                "<strong>If it does not:</strong> <em>cannot generate … `kealsql` is not installed</em> — the binary of step 5 is not on the path; <code>export KEALSQL=/path/to/kealsql</code>. <em>libpq-fe.h: No such file</em> — install <code>libpq-dev</code> (Linux) and check <code>pg_config --includedir</code>. <em>connection refused</em> or <em>authentication failed</em> — step 4.",
            ]),
    ...
    "fr": {
        ...
        "steps": [
            ...
            ("Installer PostgreSQL", [
                ...
                "PostgreSQL est un <em>serveur</em> : un programme qui tourne en arrière-plan et tient autant de bases que l'on veut, chacune avec ses tables. <code>psql</code> est le terminal qui lui parle. Sous Debian et Ubuntu le paquet démarre le serveur pour vous ; <code>pg_isready</code> dit s'il est là.",
                "<strong>Si ça ne marche pas :</strong> <em>no response</em> ou <em>connection refused</em> veut dire que le serveur ne tourne pas — <code>sudo systemctl start postgresql</code> sous Linux, <code>brew services start postgresql@17</code> sous macOS.",
            ...
            ("Créer la base", [
                "Votre première base, en deux commandes. <code>createdb blog</code> demande au serveur une base neuve et vide nommée <code>blog</code>. <code>psql -d blog -f blog.sql</code> l'ouvre et y exécute le fichier compilé : ses instructions <code>CREATE</code> font le type enum et les deux tables, et <code>psql</code> répond une ligne par instruction. Puis <code>\\dt</code> liste les tables — <code>-c</code> exécute une commande et revient au shell.",
            ], "createdb blog\npsql -d blog -f blog.sql\npsql -d blog -c '\\dt'", "CREATE TYPE\nCREATE TABLE\nCREATE TABLE\nPREPARE\nPREPARE\nPREPARE\nPREPARE\nPREPARE\nPREPARE\n         List of tables\n Schema | Name | Type  |  Owner\n--------+------+-------+--------\n public | post | table | you\n public | user | table | you\n(2 rows)", [
                "<strong>Si ça ne marche pas :</strong> <em>database \"blog\" already exists</em> — vous l'avez lancé deux fois ; <code>dropdb blog</code> la jette, lignes comprises, et vous repartez. <em>permission denied to create database</em> ou <em>role … does not exist</em> — étape 4. <em>connection to server … failed</em> — étape 3, le serveur ne tourne pas.",
                "Les instructions <code>CREATE</code> sont permanentes. Les <code>PREPARE</code> ne le sont pas : elles vivent le temps de la session <code>psql</code> qui les a exécutées. C'est pourquoi l'étape suivante les recharge, avec <code>--queries</code>, dans la session qui s'en sert — et pourquoi un programme (étape 11) prépare les siennes.",
            ]),
            ("Mettre des lignes, et les interroger", [
                "Maintenant une session <code>psql</code> dans laquelle vous restez. La première ligne, dans le shell, écrit les requêtes seules dans un fichier — <code>--queries</code> imprime les <code>PREPARE</code> sans les <code>CREATE</code>, qui échoueraient sur des tables qui existent. <code>psql -d blog</code> ouvre ensuite la base et vous donne son propre prompt, <code>blog=&gt;</code> : ce qui suit ce prompt se tape dans <code>psql</code>, et chaque commande finit par un point-virgule. <code>\\i</code> lit le fichier de requêtes dans la session ; les deux <code>INSERT</code> mettent deux utilisateurs et trois billets ; <code>EXECUTE</code> appelle une instruction préparée par son nom — les noms du fichier, en <code>snake_case</code> ; le dernier <code>SELECT</code> montre la table telle qu'elle est ; <code>\\q</code> sort.",
            ], "kealsql --queries blog.kealsql > queries.sql\npsql -d blog\n\nblog=> \\i queries.sql\nblog=> INSERT INTO \"user\" (name, email) VALUES ('ada', 'ada@x'), ('bob', 'bob@x');\nblog=> INSERT INTO post (author, editor, title, status, created) VALUES\n         (1, 2, 'Hello', 'Published', '2026-01-01'),\n         (1, NULL, 'Draft one', 'Draft', '2026-01-02'),\n         (2, NULL, 'Bob post', 'Published', '2026-01-03');\nblog=> EXECUTE by_author('ada');\nblog=> EXECUTE editor_of(2);\nblog=> EXECUTE drafts;\nblog=> SELECT id, author, title, status FROM post;\nblog=> \\q", " id | title\n----+-------\n  1 | Hello\n(1 row)\n\n name\n------\n\n(1 row)\n\n count\n-------\n     1\n(1 row)\n\n id | author |   title   |  status\n----+--------+-----------+-----------\n  1 |      1 | Hello     | Published\n  2 |      1 | Draft one | Draft\n  3 |      2 | Bob post  | Published\n(3 rows)", [
                "<strong>Si ça ne marche pas :</strong> <em>prepared statement \"by_author\" does not exist</em> — c'est une nouvelle session, les <code>PREPARE</code> de la précédente ont disparu : <code>\\i queries.sql</code> à nouveau. <em>duplicate key value violates unique constraint</em> — les lignes y sont déjà ; continuez. Un prompt devenu <code>blog-&gt;</code> attend le point-virgule oublié.",
            ...
            ], ("app.keal", "import \"./blog.kealsql\"\n\n// La base et ses tables, créées si elles manquent ; une connexion dessus.\nval db = createBlog(\"\", \"blog\")\n\nprintln(\"drafts: ${db.drafts()}\")\nfor (p in db.byAuthor(\"ada\")) {\n    println(\"#${p.id} ${p.title}\")\n}\nval editor = db.editorOf(1)\nprintln(\"editor of #1: ${editor ?: \"none\"}\")\n\n// Une modification : le brouillon sort, et sa ligne revient en record.\nval post = db.publish(2)\nprintln(\"published: ${post.headline()}\")\nprintln(\"drafts now: ${db.drafts()}\")\nfor (p in db.byAuthor(\"ada\")) {\n    println(\"#${p.id} ${p.title}\")\n}\ndb.close()"), None, [
                "<code>createBlog(\"\", \"blog\")</code> : le premier argument est une chaîne de connexion libpq — vide, elle prend les mêmes défauts que <code>psql</code> (votre utilisateur, le serveur local) ; le second est le nom de la base, créée si elle n'existe pas. Ici elle existe, avec des lignes, donc l'appel ne fait que se connecter. Chaque <code>func</code> et <code>proc</code> du fichier est une méthode sur <code>db</code>, avec les paramètres et le résultat que le fichier déclare.",
                "<code>db.drafts()</code> répond un <code>Int</code>. <code>db.byAuthor(\"ada\")</code> répond une liste de records avec <code>id</code> et <code>title</code>. <code>db.editorOf(1)</code> répond un <code>String?</code>, d'où le <code>?: \"none\"</code>. <code>db.publish(2)</code> est la modification : elle exécute l'<code>UPDATE</code> et répond la ligne changée, en <code>Post</code> — un record avec les colonnes de la table et la méthode <code>headline()</code> que la table déclare. Rien ici n'est du SQL : une colonne que le fichier n'a pas, ou un mauvais type, arrête la construction.",
            ]),
            ("Le construire et le lancer", [
                ...
                "Le second billet d'Ada était un brouillon, donc <code>byAuthor</code> ne le listait pas ; publié, il vient en premier, étant le plus récent. Relancez <code>./app</code> : la première ligne dit maintenant <code>drafts: 0</code>, parce que la modification a été écrite dans la base — ce n'est pas la mémoire du programme. <code>psql -d blog -c 'SELECT * FROM post'</code> le montre de l'autre côté, à tout moment.",
                ...
                "<strong>Si ça ne marche pas :</strong> <em>cannot generate … `kealsql` is not installed</em> — le binaire de l'étape 5 n'est pas sur le chemin ; <code>export KEALSQL=/chemin/vers/kealsql</code>. <em>libpq-fe.h: No such file</em> — installez <code>libpq-dev</code> (Linux) et vérifiez <code>pg_config --includedir</code>. <em>connection refused</em> ou <em>authentication failed</em> — étape 4.",
            ]),
